...
# Continue the remote 9373a67 scratch model with QK norm and a fresh AdamW at 3e-5.

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
cd "$repo_root"

dataset_root=${PLOT_DATASET_ROOT:?Set PLOT_DATASET_ROOT to the downloaded release}
output_dir=${OUTPUT_DIR:?Set a NEW output directory for this continuation}
staging_dir=${PLOT_CHECKPOINT_STAGING_DIR:?Set PLOT_CHECKPOINT_STAGING_DIR}
warm_start=${WARM_START:?Set the complete no-QK M3 checkpoint from run 9v021ve5}
...
if [[ "$nproc" -ne 7 || "${BATCH_SIZE:-4}" -ne 4 ]]; then
  echo "This recipe preserves the remote 7-GPU, batch-4 setup" >&2
  exit 1
fi
...
"$python_bin" -m torch.distributed.run --standalone --nproc-per-node="$nproc" \
  ...
  --wandb-name "${WANDB_NAME:-m3-qk-lr3e5-continuation-7gpu-b4}" \
