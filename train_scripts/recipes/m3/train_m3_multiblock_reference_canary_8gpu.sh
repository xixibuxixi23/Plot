...
# Canary for depth-persistent appearance conditioning.  It keeps one shared
# reference adapter, preserves its input-level use, and learns zero-started
# residual gates before DiT blocks 3, 7, and 11.
...
BASE_CHECKPOINT="${BASE_CHECKPOINT:-${PLOT_ROOT}/outputs/m3_mixed_s11_referenceonly_formal_27000_37000_20260914/step_0032750.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-${PLOT_ROOT}/outputs/m3_multiblock_reference_canary_32750_32850_20260915}"
CUDA_DEVICES="${CUDA_DEVICES:-0,1,2,3,4,5,6,7}"
FINAL_STEP="${FINAL_STEP:-32850}"
...
exec "${PLOT_ROOT}/.venv/bin/python" -m torch.distributed.run \
  ...
  --steps "${FINAL_STEP}" --save-every 100 --validate-every 1000 \
  ...
  --player-identity-negative-weight 1 --lr "${LEARNING_RATE}" --seed 3 \
  --wandb-mode offline \
  --checkpoint-staging-dir "${PLOT_ROOT}/.checkpoint_staging_multiblock_canary" \
