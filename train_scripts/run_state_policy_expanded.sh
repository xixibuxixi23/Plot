...
if [[ ! -f "$cache/summary.json" ]]; then
  "$python" dataset_toolkits/prepare_state_policy.py \
    ...
    --train-episodes 64 --val-episodes 16 --windows-per-episode 16 \
    --retain-attacks --shuffle-episodes --seed 42 --cache-workers 4
fi
"$python" train_scripts/train_state_policy.py --cache-dir "$cache" \
  --output-dir "$output" --profile "$profile" --steps 3000 --batch-size 8 \
  --validate-every 500 --workers 0 --device cuda --precision bf16 \
  --preload-cache "${extra[@]}"
echo "EXPANDED_COMPLETE=$profile"
