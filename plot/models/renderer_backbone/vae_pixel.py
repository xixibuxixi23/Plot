...
from .embeddings import RotaryEmbedding, apply_rotary_emb, PixelPatchEmbedder


#############################################

#           From Oasis WM vae.py            #
...
class Attention(nn.Module):
    def __init__(
        ...
        if qk_rms_norm:
            self.q_rms_norm = MultiHeadRMSNorm(head_dim, num_heads)
            self.k_rms_norm = MultiHeadRMSNorm(head_dim, num_heads)
    ...
    def forward(self, x):
        ...
        q = rearrange(q, "b (H W) (h d) ->  b h (H W) d", H=self.frame_height, W=self.frame_width, h=self.num_heads)
        k = rearrange(k, "b (H W) (h d) -> b h (H W) d", H=self.frame_height, W=self.frame_width, h=self.num_heads)
        v = rearrange(v, "b (H W) (h d) -> b h (H W) d", H=self.frame_height, W=self.frame_width, h=self.num_heads)

        if self.qk_rms_norm:
            q = self.q_rms_norm(q)
            k = self.k_rms_norm(k)

        q = rearrange(q, "b h (H W) d -> b h H W d", H=self.frame_height, W=self.frame_width, h=self.num_heads)
        k = rearrange(k, "b h (H W) d -> b h H W d", H=self.frame_height, W=self.frame_width, h=self.num_heads)
        v = rearrange(v, "b h (H W) d -> b h H W d", H=self.frame_height, W=self.frame_width, h=self.num_heads)
...
class ViTVae(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # patch
        self.patch_embed = PixelPatchEmbedder(input_height, input_width, patch_size, 3, enc_dim)
