"""Full-size CUDA/DDP smoke for the text-conditioned v2 policy."""
import argparse
import json
...
from plot.models.state_policy_text_v2 import TextStatePolicyV2, TextStatePolicyV2Args
...
def main():
    ...
    model = TextStatePolicyV2(TextStatePolicyV2Args(402, 13)).to(device)
    batch = 1
    data = dict(
        ...
        shared_text=torch.randn(batch, 12, 768, device=device),
        shared_text_mask=torch.ones(batch, 12, dtype=torch.bool, device=device),
        current_text=torch.randn(batch, 16, 768, device=device),
        current_text_mask=torch.ones(batch, 16, dtype=torch.bool, device=device))
    camera = raster_camera(np.array([0., 0., 1.5]), np.array([1., 0., 0.]),
                           1.2, np.zeros(3))
    data["raster_camera"] = torch.from_numpy(camera).to(device)[None]
    ...
    for _ in range(args.steps):
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            loss, parts = model.loss(
                logits, actions, horizon_weights=torch.linspace(1, 0.25, 8, device=device))
    ...
    torch.cuda.synchronize()
    result = dict(rank=rank, world_size=world,
                  parameters=sum(parameter.numel() for parameter in model.parameters()),
                  seconds=time.monotonic() - started,
                  peak_mb=torch.cuda.max_memory_allocated() / 2 ** 20,
                  m3_depth_stack_equivalent=True, finite_gradients=True,
                  text_cross_attention=True, split_attack_place=True, all_reduce=True)
    results = [None] * world
    if world > 1:
