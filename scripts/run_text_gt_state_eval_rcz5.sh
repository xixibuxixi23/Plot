...
CUDA_VISIBLE_DEVICES=0 "$ROOT/envs/plot-env-py311-cu128/bin/python" \
  ...
  --m2-checkpoint "$ROOT/downloads/plot-checkpoints-main/m2/m2_s02_s10_attack_v7/deployed_checkpoint.pt" \
  --m4-checkpoint "$ROOT/runs/m4-state/text-hier-pointer-20k-20260923-v1/best-val-loss.pt" \
  --output "$ROOT/runs/m4-state/text-hier-pointer-20k-20260923-v1/eval/m2-gt-state-s07-2239-final20k.json" \
  --target 0 \
  --max-blocks 60 \
  --modes m4_gt_state \
  --model-start 0
